...
export class LessonStateService {
  ...
  protected readonly lessonId$ = defer(() => this.lessonStateSubject.pipe(selectState('lessonId')));
  ...
  init(lessonId: string): Unsubscribable {
    ...
    const loadLesson = this.addState('lesson',
      combineLatest([
        this.block$,
        this.lessonId$
      ]).pipe(
        map(([block, lessonId]) => block.getLesson(lessonId)),
